...
восемь, поэтому запуск не заводит задание вовсе, а отвечает `501` и называет,
чего не хватает: задание с прогрессом 100 над нетронутым звуком — это зеленая
галочка за несделанную работу.

Статус задания двигают переходы, а не присвоение: недопустимая пара отвечает
`409 invalid_job_transition` до записи в базу. Перечень пар — в
`ALLOWED_TRANSITIONS`.
"""
...
@router.post(
    ...
    description=(
        ...
        "Пока ни один шаг конвейера не подключен, запуск отвечает `501` и перечисляет, "
        "чего не хватает: задание, завершенное без единой выполненной работы, не заводится."
    ),
...
@router.get(
    ...
    description=(
        ...
        "Прогресс считается только по выполняемым шагам."
    ),
...
@router.post(
    ...
    description=(
        "Повторяется только упавшее задание: `error` -> `queued` — единственный переход "
        "обратно в работу. Завершенное задание в работу не возвращается, повторная "
        "обработка заводится новым запуском."
    ),
