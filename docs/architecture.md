...
### Как это реализовано (2026-09-10)

Контракт выгружается сервером в `docs/api/openapi.json` — на стороне сервера
это держит тест `test_exported_openapi_is_current`. Типы клиента
генерируются из него в `src/services/api/schema.d.ts` командой
`npm run api:types`; рассинхрон ловит тест `src/services/api/schema.test.ts`.

Руками во фронтенде остаются только рантайм-декодеры: генератор дает типы,
но не проверку данных на границе. Декодер типизирован генерированным типом,
поэтому расхождение между ним и контрактом — ошибка компиляции.

Почему это записано отдельно. Первая реализация клиента была написана руками
против этого решения: 1117 строк типов, продублировавших контракт. Сверка
показала расхождение в 24 типах из 49. Конкретика, чтобы было понятно, чем
это грозит: сервер объявлял `maxDurationSeconds` необязательным, клиент —
обязательным; декодер выгрузки материала читал два поля из пяти. Ни то, ни
другое не видно на моках и вылезло бы на первом реальном ответе сервера.
