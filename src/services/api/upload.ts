/**
 * Отправка файла в хранилище по выданной ссылке — с прогрессом.
 *
 ...
 *
 * Файл идет прямо в хранилище, минуя наш API (`server/app/api/schemas/uploads.py`):
 * иначе каждая песня занимала бы процесс приложения на все время передачи.
 */
...
export const uploadFileToTarget = (
  ...
  return new Promise<ApiResult<null>>((resolve) => {
    ...
    xhr.addEventListener("load", () => {
      ...
      // Хранилище отвечает своим форматом ошибки, не нашим конвертом. Тело
      // наружу не отдаем: в нем бывает служебная диагностика провайдера.
      finish(
        fail<null>({
          kind: "http",
          status: xhr.status,
